...
class Search extends CI_Controller {
	...
	public function index(){
		...
		$jumlah_data = $this->m_main->getLike('content',array('judul','tgl_terbit','isi'),$search,3,$from,'tgl_terbit','DESC')->num_rows();
		$this->paggi('_lod/search/index/',$jumlah_data,1);
		
		$data = array(
				'title'      => 'Page tugas',
				'content' 	 => $this->m_main->getLike('content',array('judul','tgl_terbit','isi'),$search,3,$from,'tgl_terbit','DESC')->result(),
				'fasilitas'  => $this->m_main->getData_sort('content','6','tgl_terbit','DESC', array('kategori' => 'fasilitas'))->result(),
				'pagination' => $this->pagination->create_links(),
			);
		$this->parser->parse("search.tpl",$data);
	}
	public function view(){
		...
		$this->parser->parse("dt_view.tpl",$data);
	}
